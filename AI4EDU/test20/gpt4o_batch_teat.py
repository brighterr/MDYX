...
def send_request(user_id: int):
    ...
    try:
        ...
        if response.choices:
            ...
            insert_message(username, "assistant", error)
...
def run_batch_test():
    ...
    df_all.to_csv("gpt4o_stress_test_results.csv", index=False)
    
    # 保存汇总结果
    df_summary = pd.DataFrame(summary_results)
    df_summary.to_csv("gpt4o_batch_test_results.csv", index=False)
...
def plot_results():
    ...
    df = pd.read_csv("gpt4o_batch_test_results.csv")
    ...
    plt.savefig("gpt4o_batch_test_results.png", dpi=300, bbox_inches='tight')
    plt.show()
