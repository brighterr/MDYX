...
HEADERS = {
    ...
    "Authorization": "Bearer sk-0518752ba5c545e2a22555406d10070f"  # 你的 API Key
}
MODEL = "deepseek-chat"
...
def run_batch_test():
    ...
    df_all.to_csv("deepseek_stress_test_results.csv", index=False)
    
    # 保存汇总结果
    df_summary = pd.DataFrame(summary_results)
    df_summary.to_csv("deepseek_batch_test_results.csv", index=False)
...
def plot_results():
    ...
    df = pd.read_csv("deepseek_batch_test_results.csv")
    ...
    plt.savefig("deepseek_batch_test_results.png", dpi=300, bbox_inches='tight')
    plt.show()
